...
positionx = text(0.6,-4.2,'0.00','FontSize',22,'Color', 'b');
positiony = text(2.2,-4.2,'0.00','FontSize',22,'Color', 'g');
%Target Pt.
...
set(f, 'units', 'inches', 'position', [0 0 10 9])
set(f,'Color',[1,1,1]);
...
%     
    if(figData.yend <= -1.5)
        p.Fy = p.collision*abs((figData.yend-zyold)/dt)+0.1*p.collision*abs(figData.yend+1.5);
        if(abs(zxold-figData.xend)>0.005)
            p.Fx = 50*(zxold-figData.xend);
        else
            p.Fx=0;
        end
        disp("collision");
    ...
    set(positionx,'string',strcat(num2str(figData.xend,2)));
    set(positiony,'string',strcat(num2str(figData.yend,2)));
